...
public class ModData {
    ...
    @JsonProperty("files")
	public List<ModFile> files = null;
	...
	public List<ModFile> getFilesForVersion(String version) {
		return files.stream().filter(p -> p.version.equals(version) || p.versions.contains(version))
				.sorted(Comparator.comparing(ModFile::getDate).reversed()).collect(Collectors.toList());
	}

	public List<ModFile> getBetaFilesForVersion(String version) {
		return getFilesForVersion(version).stream().filter(f -> "beta".equals(f.type)).collect(Collectors.toList());
	}

	public List<ModFile> getReleaseFilesForVersion(String version) {
